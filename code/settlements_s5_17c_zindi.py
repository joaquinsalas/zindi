import os
os.environ["CUDA_VISIBLE_DEVICES"] = "3"  # Updated to use GPU 2
import h5py
import numpy as np
import pandas as pd
import torch
...
def create_submission(test_predictions):
    id_map = pd.read_csv('../../2024.09.04zindi/data/id_map.csv')
    ...
    submission_file = f'../../2024.09.04zindi/submissions/best_s5_model_submission.csv'
...
# Main function
def main():
    h5_file_test = '../../2024.09.04zindi/data/test_data.h5'
    batch_size = 512

    # Load the pre-trained model
    device = 'cuda' if torch.cuda.is_available() else 'cpu'
    model = S5Model(d_input=17, d_output=1, d_model=1024, n_layers=2, dropout=0.1, prenorm=True)
    model.load_state_dict(torch.load('settlements/best_s5_model_17c.pth', map_location=device))
